            ...
            <div class="lg:col-span-2 w-full p-8 max-w-lg max-md:max-w-lg mx-auto">
                <form method="POST" action="{{ route('authenticate') }}">
                    ...
                    <div class="mb-8">
                        ...
                        <p class="text-[15px] mt-6 text-slate-600">Don't have an account <a href="javascript:void(0);"
                                class="text-blue-600 font-medium hover:underline ml-1 whitespace-nowrap">Register
                                here</a></p>
                    </div>

                    @if ($errors->any())
                        <div class="flex items-start bg-red-100 text-red-800 p-4 rounded-lg relative mb-5"
                            role="alert">
                            <div class="inline-block">
                                <span class="font-semibold text-[15px] inline-block mr-3">Error!</span>
                                @foreach ($errors->all() as $error)
                                    <span
                                        class="block text-sm font-medium sm:inline max-sm:mt-2 mr-4">{{ $error }}</span>
                                @endforeach
                            </div>
                        </div>
                    @endif
